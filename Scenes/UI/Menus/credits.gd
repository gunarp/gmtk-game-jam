...
func _ready():
  ...
  # scroll_tween.tween_property($RichTextLabel,

  fade_tween = create_tween()
  # tween.tween_property(label, "position:y", -label.size.y, 30.0)
  fade_tween.tween_property($Fade, "modulate:a", 0.0, FADE_TIME) # fade out over 2 seconds
  # tween.parallel().tween_property(label, "position:y", end_y, scroll_duration)

  fade_tween.tween_callback(func():
    var fade_out = create_tween()
    fade_out.tween_property(fade, "modulate:a", 1.0, FADE_TIME)
